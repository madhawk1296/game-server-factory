variable "network_name" {
  description = <<-EOT
    Docker network the worlds join. Owned by the caller, not the module: the
    object store lives on it too, and a module that created its own network
    would force the caller to depend on the module that depends on the caller.
  EOT
  type        = string
}
